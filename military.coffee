...
class MilitaryManager extends ModuleManager
        ...
        init: (gameState) ->
                civ = gameState.playerData.civ;
                if civ of @uCivCitizenSoldier
                        @uCitizenSoldier = @uCivCitizenSoldier[civ]
                        @uAdvanced = @uCivAdvanced[civ]
                        @uSiege = @uCivSiege[civ]
                        @bAdvanced = @bCivAdvanced[civ]
                for unitType in  [ @uSiege, @uAdvanced, @uCitizenSoldier ]
                        for i of unitType
                                unitType[i] = gameState.applyCiv(unitType[i])
        ...
        update: (gameState) ->
                myUnits = gameState.getOwnEntities()
